This section corresponds to transaction [ITI-105] of the IHE Technical Framework. Transaction [ITI-105] is used by the Document Source and Document Recipient Actors. The Simplified Publish [ITI-105] transaction is used to transmit a set of documents and associated metadata.
...
This message uses the HTTP POST method on the target Simplified Publish endpoint to convey the metadata and the document(s) as a FHIR transaction.
...
The Document Source shall initiate a FHIR “transaction” using a “create” action by sending an HTTP POST request method composed of a FHIR Bundle Resource containing: one DocumentReference Resources; and zero or more Binary Resources to the Document Recipient. Refer to [ITI TF-3: 4.5.1](32_fhir_maps.html) for details on the FHIR Resources and how Document Sharing metadata attributes are mapped. 
...
###### 2:3.105.4.1.2.1 Bundle Resources
...
The FHIR Bundle.implicitRules shall have the value [Simplified Publish](StructureDefinition-IHE.MHD.Simplified.PublishBundle.html): `https://profiles.ihe.net/ITI/MHD/StructureDefinition/IHE.MHD.Simplified.PublishBundle` indicating ITI-105.

The FHIR Bundle.meta.profile may have the value [Simplified Publish](StructureDefinition-IHE.MHD.Simplified.PublishBundle.html): `https://profiles.ihe.net/ITI/MHD/StructureDefinition/IHE.MHD.Simplified.PublishBundle`
...
The Document Source shall populate accurate .hash and .size for the raw document content artifact, that has been base64encoded in the Binary.data element.  
...
All DocumentReference.subject, and List.subject values shall be a Reference to a FHIR Patient Resource. The Patient Reference shall be a Patient Resource hosted on a commonly accessible server. 
...
On receipt of the Simplified Publish Request Message, the Document Recipient shall validate the resources provided, create a SubmissionSet derived off of the DocumentReference, and respond with one of the HTTP codes defined in the response [Message Semantics](#23105412-message-semantics). 
...
If the Document Recipient encounters any errors or if any validation fails, the Document Recipient shall return an error, as documented in [Provide Document Bundle Response Message](#2310542-simplified-publish-response-message). If appropriate, it shall use error codes from [ITI TF-3: Table 4.2.4.1-2](https://profiles.ihe.net/ITI/TF/Volume3/ch-4.2.html#4.2.4.1).
