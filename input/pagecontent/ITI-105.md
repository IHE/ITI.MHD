...
The Simplified Publish message is sent to the DocumentReference at the base URL as defined in FHIR. See [FHIR http.html]({{site.data.fhir.path}}http.html) for the definition of “HTTP” access methods and “base”.
```
...
The Document Recipient returns a HTTP Status code appropriate to the processing outcome, conforming to the create transaction specification requirements as specified in [FHIR http.html#create]({{site.data.fhir.path}}http.html#create). 
