...
Where values provided are References to FHIR resources (e.g. DocumentReference.encounter), the Reference should be converted to an Identifier for recording in the Document Sharing ebRIM using the ITI Volume 3 [Metadata Attribute Data Types](https://profiles.ihe.net/ITI/TF/Volume3/ch-4.2.html#4.2.3.1.7). The conversion may be by using a business `.identifier`. There is not an obvious coversion of a reference `.id` to CXi identifier.
